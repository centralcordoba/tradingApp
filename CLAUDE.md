...
- **Fuentes**: (A) señales del Pine con `decision=ENTER` vía SSE `/signals/stream` + catch-up por id, orden a mercado con SL/TP del motor; (B) marco de Zonas en OPERAR+fuerte vía poll `/api/zones` 5 min (misma semántica de transición+cooldown que las alertas del frontend, y skip si `data_age_minutes>10`).
