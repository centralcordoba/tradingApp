...
- **Ledger local en CSV** (`bridge/trades.csv`): además del POST a la DB (best-effort, puede fallar si Render está caído), cada trade se registra en un CSV local — una fila por trade con `ticket, opened_at, symbol, side, source, lots, entry_price, sl_price, tp_price, tp1_price, risk_usd, rrr, strength, score, cross_state, session, dry_run, partial_done, be_moved, result, exit_price, pnl_usd, closed_at`. Se hace append al abrir y se actualiza in-place al tomar el parcial 1R y al cerrar. Siempre disponible, se analiza directo en Excel/pandas. Gitignoreado (es data). Los dry-run quedan como fila abierta sin resultado (no hay posición real que cerrar).
